...
const AthleticsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    setPlayers([
      { id: '1', name: 'John Sprinter', team: 'Bong Sprinters', county: 'Bong', position: '100m/200m', status: 'verified', registrationDate: '2025-01-10', licenseNumber: 'ATHL-001' },
      { id: '2', name: 'Mike Jumper', team: 'Lofa Jumpers', county: 'Lofa', position: 'Long Jump', status: 'pending', registrationDate: '2025-01-12', licenseNumber: 'ATHL-002' },
      { id: '3', name: 'Sarah Thrower', team: 'Montserrado Throwers', county: 'Montserrado', position: 'Shot Put', status: 'verified', registrationDate: '2025-01-08', licenseNumber: 'ATHL-003' },
      { id: '4', name: 'David Runner', team: 'Nimba Runners', county: 'Nimba', position: '400m', status: 'rejected', registrationDate: '2025-01-15', licenseNumber: 'ATHL-004' },
      { id: '5', name: 'Lisa Hurdler', team: 'Grand Bassa Hurdlers', county: 'Grand Bassa', position: '110m Hurdles', status: 'pending', registrationDate: '2025-01-11', licenseNumber: 'ATHL-005' }
    ]);

    setComments([
      { id: '1', playerId: '2', playerName: 'Mike Jumper', comment: 'Height verification needed - listed height seems incorrect', status: 'pending', submittedDate: '2025-01-15' },
      { id: '2', playerId: '4', playerName: 'David Runner', comment: 'Medical clearance required for ankle injury', status: 'rejected', reason: 'Insufficient medical documentation', submittedDate: '2025-01-14' },
      { id: '3', playerId: '5', playerName: 'Lisa Hurdler', comment: 'Age verification needed - birth certificate unclear', status: 'pending', submittedDate: '2025-01-13' }
    ]);
  }, []);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border-l-4 border-green-500">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Verified Athletes</p>
  ...
  const renderPlayerVerification = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-gray-900">Athletics Player Verification</h3>
        <div className="text-sm text-gray-600">
          Discipline: <span className="font-semibold text-yellow-600">Athletics</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="Bong Sprinters">Bong Sprinters</option>
              <option value="Lofa Jumpers">Lofa Jumpers</option>
              <option value="Montserrado Throwers">Montserrado Throwers</option>
              <option value="Nimba Runners">Nimba Runners</option>
              <option value="Grand Bassa Hurdlers">Grand Bassa Hurdlers</option>
            </select>
          </div>
          <div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        className="w-32 px-2 py-1 text-xs border border-gray-300 rounded focus:ring-yellow-500 focus:border-yellow-500"
    ...
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="flex">
        ...
        <div className="w-64 bg-white shadow-sm min-h-screen">
          <div className="p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Athletics Dashboard</h2>
            <p className="text-sm text-gray-600 mb-4">Athletics Federation</p>
            <nav className="space-y-2">
              <button
                onClick={() => setActiveTab('overview')}
                className={`w-full flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === 'overview' ? 'bg-yellow-100 text-yellow-700' : 'text-gray-600 hover:bg-gray-100'
                }`}
              >
              ...
              <button
                ...
                className={`w-full flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === 'players' ? 'bg-yellow-100 text-yellow-700' : 'text-gray-600 hover:bg-gray-100'
                }`}
              >
                <Lightning className="w-5 h-5 mr-3" />
                Player Verification
              </button>
              <button
                onClick={() => setActiveTab('comments')}
                className={`w-full flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === 'comments' ? 'bg-yellow-100 text-yellow-700' : 'text-gray-600 hover:bg-gray-100'
                }`}
              >
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                {activeTab === 'players' ? 'Athletics Player Verification' : 
                 activeTab === 'comments' ? 'Comments & Observations' : 'Overview'}
              </h1>
              <p className="text-gray-600 mt-1">
                {activeTab === 'players' ? 'Verify athletes and submit comments for backend approval' :
                 activeTab === 'comments' ? 'Track submitted comments and observations' :
                 'Athletics Federation Dashboard Overview'}
              </p>
            </div>
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-500">Role: Athletics Federation Official</span>
              <button
                onClick={() => router.push('/')}
